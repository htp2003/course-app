import { Form, Typography, Divider, Collapse, Tag, Empty, List, Space } from "antd";
import {
    VideoCameraOutlined,
    FileTextOutlined,
    DesktopOutlined,
...
const getLessonIcon = (type: string) => {
    switch (type) {
        ...
        case 'document': return <FileTextOutlined className="text-orange-500" />;
        case 'slide': return <DesktopOutlined className="text-green-500" />;
...
const getLessonTypeTag = (type: string) => {
    switch (type) {
        case 'video': return <Tag color="blue">Video</Tag>;
        case 'document': return <Tag color="orange">Tài liệu</Tag>;
        case 'slide': return <Tag color="green">Slide</Tag>;
                        ...
                        <div className="flex items-start gap-3 px-2">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="text-xs text-gray-400 mt-1">
                                    {lesson.type === 'video' && (lesson.videoUrl ? `Link: ${lesson.videoUrl}` : 'Chưa nhập link video')}
                                    {(lesson.type === 'document' || lesson.type === 'slide') && 'Đã đính kèm file tài liệu'}
                                </div>
                                { }
                                {lesson.quizzes && lesson.quizzes.length > 0 && (
                                    <div className="mt-3 bg-indigo-50 rounded-md p-2">
                                        <div className="text-xs font-bold text-indigo-600 mb-2 uppercase flex items-center gap-1">
                                            <CheckSquareOutlined /> Bài kiểm tra ({lesson.quizzes.length})
                                        </div>
                                        <ul className="list-disc list-inside text-sm text-gray-600 pl-1 space-y-1">
                                            {lesson.quizzes.map((quiz: IQuiz, qIdx) => (
                                                <li key={qIdx}>
                                                    {quiz.title} <span className="text-gray-400 text-xs">({quiz.questions?.length || 0} câu hỏi)</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
