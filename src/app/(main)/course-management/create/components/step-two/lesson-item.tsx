...
import { Form, Input, Select, InputNumber, Row, Col, Card, Button, Tooltip } from "antd";
import { DeleteOutlined, FileTextOutlined, VideoCameraOutlined, DesktopOutlined } from "@ant-design/icons";
import { LESSON_TYPES, UPLOAD_CONFIG } from "../../../common/constants";
...
const getUploadConfig = (type: LessonTypeType) => {
...
export const LessonItem = memo(({ chapterIndex, lessonIndex, remove }: Props) => {
        ...
        <div className="flex-1">
          ...
          <Form.Item
            ...
            shouldUpdate={(prev: ICreateCourseForm, curr: ICreateCourseForm) => {
              const prevType = prev.chapters?.[chapterIndex]?.lessons?.[lessonIndex]?.type;
              const currType = curr.chapters?.[chapterIndex]?.lessons?.[lessonIndex]?.type;
            ...
            {({ getFieldValue }) => {
                ...
                return (
                  ...
                  >
                    <CommonFileUpload
                      ...
                      helperText={`${config.HELPER_TEXT} (Tối đa ${config.maxCount} file)`}
                      value={[]}
                      listType="picture"
                      height={160}
              ...
              return (
                <Form.Item
                  ...
                  help={<span className="text-xs text-gray-400">Hỗ trợ: Youtube, Vimeo, hoặc link .mp4 trực tiếp</span>}
                >
                  <Input
